...
//	private static final int ENFORCES_UNIQUE_CONSTRAINT = 4096;
	...
	public Class<?> determineJavaType(DatabaseColumn argDatabaseColumn) {
		...
		if (argDatabaseColumn instanceof SQLServerColumn) {
			SQLServerColumn lclSScolumn = (SQLServerColumn) argDatabaseColumn;
...
//		final boolean lclN = argDatabaseColumn.isNullable();
	...
	protected ArrayList<DatabaseColumn> loadColumns() throws SQLException {
		String lclSQL = 
			"SELECT	C.TABLE_CATALOG, C.TABLE_SCHEMA, C.TABLE_NAME, C.COLUMN_NAME, C.COLUMN_DEFAULT, C.IS_NULLABLE, C.DATA_TYPE, C.CHARACTER_MAXIMUM_LENGTH, C.NUMERIC_PRECISION, C.NUMERIC_SCALE, C.DOMAIN_NAME, C2.status " +
...
//					}
	...
	//				boolean lclDebug = lclConstraintName.contains("Feng");
					
					if ((lclFK == null) || (!lclFK.getName().equals(lclConstraintName))) {
	//					if (lclDebug) {
	//						System.out.println("Creating a new foreign key");
	//					}
	...
	//				if (lclDebug) {
	//					System.out.println("Adding column to " + lclFK);
	//				}
	...
	//				if (lclDebug) {
	//					System.out.println(lclRS.getString("source_column_name") + " -> " + lclRS.getString("target_column_name") + " (" + lclFK.getSourceKey().getColumnNameList().size() + " / " + lclFK.getTargetKey().getColumnNameList().size() + ")");
	//				}
				}
	...
	protected ReferentialAction determineReferentialAction(int argAction) {
		...
		default: throw new IllegalStateException(); // TODO: Proper error message
		}
	}
		...
		String lclSQL =
			"SELECT SI.name as index_name, SI.status, T.name as table_name, C.name as column_name, IC.keyno, SI2.filter_definition " +
				...
				"WHERE SI.indid between 1 and 254 " +
					"AND (SI2.has_filter = 0 OR (SI2.has_filter = 1 AND SI2.filter_definition LIKE '% IS NOT NULL)')) " + // FIXME: OpalCaches don't know how to handle filtered indexes that don't simply filter out NULL keys; this is a hack, no two ways about it.
