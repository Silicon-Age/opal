...
public abstract class AbstractDatabaseIdentityOpalFactory<U extends IdentityUserFacing, O extends IdentityOpal<U>> extends AbstractIdentityOpalFactory<U, O> {
	 ...
	 */
	protected void deleteInternal(DatabaseTransactionParameter argTP, O argOpal) {
		if (argOpal.isNew()) {
			return;
		}
		...
		if (lclParameters != null) {
			...
			for (int lclI = 0; lclI < lclParameters.length; ++lclI) {
				...
				if (lclO instanceof java.time.LocalDate) {
					lclParameters[lclI] = java.sql.Date.valueOf((LocalDate) lclO);
				} else if (lclO instanceof java.time.LocalDateTime){
					lclParameters[lclI] = java.sql.Timestamp.valueOf((LocalDateTime) lclO);
				} else if (lclO instanceof com.opal.types.UTCDateTime){
					lclParameters[lclI] = java.sql.Timestamp.valueOf(((UTCDateTime) lclO).toLocalDateTime());
				} else if (lclO instanceof StringSerializable) {
					lclParameters[lclI] = ((StringSerializable) lclO).toSerializedString();
				} else {
...
//	protected ArrayList<O> loadAll(/* String argSchemaName, String argTableName */) {
//		ArrayList<O> lclAL = new ArrayList<O>();
//		load(getFullyQualifiedTableName(), null, null, null, lclAL);
//		return lclAL;
//	}
	...
	protected void adjustParameters(Object[] argParameters) {
		...
		for (int lclI = 0; lclI < argParameters.length; ++lclI) {
			...
			} else if (lclO instanceof UTCDateTime) {
...
//			System.out.println();
	...
	protected Object translateJavaObjectToSQLValue(Object argO) { // THINK: Is this actually really slow?  SHould we check Strings and Numbers first?
		...
		if (argO instanceof LocalDate) {
			lclDatabaseValue = java.sql.Date.valueOf((LocalDate) argO);
		} else if (argO instanceof LocalTime) {
			lclDatabaseValue = java.sql.Time.valueOf((LocalTime) argO);
		} else if (argO instanceof LocalDateTime) {
			lclDatabaseValue = java.sql.Timestamp.valueOf((LocalDateTime) argO);
		} else if (argO instanceof UTCDateTime) {
			lclDatabaseValue = java.sql.Timestamp.valueOf(((UTCDateTime) argO).toLocalDateTime());
		} else if (argO instanceof OffsetDateTime) {
//			lclDatabaseValue = DateTimeFormatter.ISO_OFFSET_DATE_TIME.format((OffsetDateTime) argO);
			 ...
			 */
			lclDatabaseValue = java.sql.Timestamp.valueOf(((OffsetDateTime) argO).toLocalDateTime());
		} else if (argO instanceof StringSerializable) {
			lclDatabaseValue = ((StringSerializable) argO).toSerializedString();
		} else {
...
//	protected String getLargeFieldName(@SuppressWarnings("unused") int argLargeFieldIndex) {
//		return "database";
//	}
//	
//	protected byte[] loadLargeObjectContent(O argOpal, int argLargeFieldIndex) {
//		Connection lclConnection = null;
//		ResultSet lclRS = null;
//		try {
//			// TODO:  If we have an active Transaction, we should use it. */
//			lclConnection = getDataSource().getConnection();
//			lclRS = DatabaseUtility.select(
//				lclConnection,
//				new String[] { getLargeFieldName(argLargeFieldIndex) },
//				getFullyQualifiedTableName(),
//				getPrimaryKeyWhereClauseColumns(),
//				argOpal.getPrimaryKeyWhereClauseValues()
//			);
...
//			}
//			Blob lclBlob = lclRS.getBlob(1);
//			if (lclRS.next() == true) {
//				throw new IllegalStateException("Multiple rows returned.");
//			}
//			if (lclBlob == null) {
//				return null;
//			} else {
//				long lclLength = lclBlob.length();
//				if (lclLength > Integer.MAX_VALUE) {
//					throw new IllegalStateException("BLOB length is larger than Integer.MAX_VALUE and will therefore not fit in an array.");
//				} else {
//					return lclBlob.getBytes(0L, (int) lclLength);
//				}
//			}
//		} catch (SQLException lclE) {
//			throw new PersistenceException("Unable to load BLOB field " + argLargeFieldIndex + " for " + this, lclE);
//		} finally {
//			DatabaseUtility.cleanUp(lclRS, lclConnection);
//		}
//	}
...
//	protected void commitLargeObjectPhaseOne(LargeObject argLO, O argOpal, int argLargeFieldIndex, TransactionParameter argTP) {
//		/* TODO:  Write out the data */
//		return;
//	}
//	
//	@SuppressWarnings("unused")
//	protected void commitLargeObjectPhaseTwo(LargeObject argLO, O argOpal, int argLargeFieldIndex, TransactionParameter argTP) {
//		return;
//	}
}
